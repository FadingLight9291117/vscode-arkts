...
- **Editor title-bar build/run/sign** (`src/runApp.ts`): `arkts.buildApp` / `arkts.runApp` / `arkts.signApp` find the project root by walking up from the active `.ets` file for the **topmost** `oh-package.json5` (`findProjectRoot` — deliberately tolerant of projects without `hvigorw`), then shell out to `devecocli build` / `devecocli run` / `devecocli signature generate` with output streamed to the "ArkTS Run" output channel. `buildApp`/`runApp` run directly with the build version preset by the title-bar mode button (`arkts.selectBuildMode`), stored per workspace in workspaceState key `arkts.lastBuildMode` (`default` → no extra args; `debug`/`release` → `--build-mode`); unset → `default` (`pickBuildMode`, `src/runApp.ts`). `devecocli` (`@deveco/deveco-cli`) is installed **globally** on demand at activation (not into global storage, unlike the language server); `arkts.installDevecoCli` is the manual entry point.
