...
- **LSP client** (`vscode-languageclient`): connects to `@arkts/language-server` via IPC. The server is **not bundled in the VSIX** — `ensureLanguageServer()` (`src/runApp.ts`) installs `@arkts/language-server@1.3.10` on demand to the extension's global storage dir (`<globalStorage>/language-server/node_modules/...`), and the bin path is resolved from there. Completion, hover, go-to-definition, references, and diagnostics all come from the LSP — there are no hand-rolled providers in `src/providers/`. Requires `ets.sdkPath` to be set or the LSP won't start.
...
- `vscode-languageclient` is marked **external** in `esbuild.js` (not bundled) and is the only entry in `dependencies`. `@arkts/language-server` was removed from dependencies to keep the VSIX at ~545 KB — it's installed on demand to the extension's global storage dir. Other runtime deps (zod, MCP SDK) stay in `devDependencies` and are bundled.
...
Companion doc: [AGENTS.md](AGENTS.md) at the repo root is the agent-focused architecture guide — when adding MCP tools or changing the LSP setup, update both files to stay in sync.
...
Use conventional-commit subjects with a concise Chinese summary, then an indented bullet list of details (2-space indent before each `-`):

```plain
feat: 持久化工作记录并修复计薪数据一致性

  - 引入 v3 状态模型，以不可变工作记录替代单日聚合计时器
  - 自动迁移 v1/v2 浏览器数据，保留既有设置与计时状态
  - 跨午夜自动拆分会话记录并继续计时，不再丢失工时
```

Common prefixes: `feat:` (feature), `fix:` (bug fix), `refactor:`, `docs:`, `test:`, `build:`/`chore:`. Keep each commit focused; one logical change per commit. PRs explain the change and user impact, link issues, and report build/lint/test results. Record every user-visible feature or bug fix in the `未发布`/`Unreleased` section of `CHANGELOG.md` as soon as it is implemented; move those entries into a versioned section when releasing. Include desktop + mobile screenshots for visual changes.
